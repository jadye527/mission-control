...
exports.TOOL_GROUP_LABELS = exports.MODEL_TIERS = exports.AGENT_TEMPLATES = void 0;
exports.getEffectiveToolGroups = getEffectiveToolGroups;
...
function getEffectiveToolGroups() {
    const merged = {};
    for (const [key, tools] of Object.entries(TOOL_GROUPS)) {
        merged[key] = [...tools];
    }
    for (const provider of (0, plugins_1.getPluginToolProviders)()) {
...
function getTemplate(type) {
    return exports.AGENT_TEMPLATES.find(t => t.type === type);
}
/** Build a full OpenClaw agent config from a template + overrides */
function buildAgentConfig(template, overrides) {
    const config = structuredClone(template.config);
    config.identity.name = overrides.name;
    if (overrides.emoji)
        config.identity.emoji = overrides.emoji;
    if (overrides.theme)
        config.identity.theme = overrides.theme;
    if (overrides.model)
        config.model.primary = overrides.model;
    if (overrides.workspaceAccess)
        config.sandbox.workspaceAccess = overrides.workspaceAccess;
    if (overrides.sandboxMode)
        config.sandbox.mode = overrides.sandboxMode;
    ...
    return Object.assign({ id: overrides.id, name: overrides.name, workspace: overrides.workspace, agentDir: overrides.agentDir }, config);
}
