...
exports.resolveCoordinatorDeliveryTarget = resolveCoordinatorDeliveryTarget;
function normalizeName(value) {
    return String(value || '').trim().toLowerCase();
}
function normalizeOpenClawId(value) {
...
function resolveConfiguredCoordinatorTarget(preferredTarget, allAgents, sessions) {
...
function resolveCoordinatorDeliveryTarget(params) {
    var _a;
    const normalizedTo = normalizeName(params.to);
    const normalizedCoordinatorAgent = normalizeName(params.coordinatorAgent);
    const explicitSessionKey = ((_a = params.explicitSessionKey) === null || _a === void 0 ? void 0 : _a.trim()) || null;
    const buildResult = (agent, resolvedBy) => {
        var _a, _b;
        const openclawAgentId = getConfigOpenClawId(agent) || normalizeOpenClawId(agent.name);
        const sessionKey = explicitSessionKey ||
            ((_a = agent.session_key) === null || _a === void 0 ? void 0 : _a.trim()) ||
            ((_b = findSessionForAgent(agent, params.sessions)) === null || _b === void 0 ? void 0 : _b.key) ||
            null;
        return {
    ...
    if (normalizedTo === normalizedCoordinatorAgent) {
        ...
        if (mainSession) {
            ...
            return {
                deliveryName: (matchingAgent === null || matchingAgent === void 0 ? void 0 : matchingAgent.name) || mainSession.agent,
                sessionKey: explicitSessionKey || mainSession.key || null,
                openclawAgentId: getConfigOpenClawId(matchingAgent || { name: mainSession.agent }) ||
