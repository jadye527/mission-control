...
function parseDayName(input) {
    var _a;
    return (_a = DAY_MAP[input.toLowerCase()]) !== null && _a !== void 0 ? _a : null;
}
function parseTimeExpr(input) {
    var _a;
    ...
    const ampm = (_a = match[3]) === null || _a === void 0 ? void 0 : _a.toLowerCase();
    if (ampm === 'pm' && hour < 12)
        hour += 12;
    if (ampm === 'am' && hour === 12)
        hour = 0;
...
export function isCronDue(cronExpr, nowMs, lastSpawnedAtMs) {
    ...
    // Check minute
    if (!matchesCronField(minExpr, now.getMinutes()))
        return false;
    // Check hour
    if (!matchesCronField(hourExpr, now.getHours()))
        return false;
    // Check day of week
    if (!matchesCronField(dowExpr, now.getDay()))
        return false;
    // Prevent duplicate spawn within same minute
    if (lastSpawnedAtMs > 0) {
        const lastSpawn = new Date(lastSpawnedAtMs);
        if (lastSpawn.getFullYear() === now.getFullYear() &&
...
function matchesCronField(expr, value) {
    if (expr === '*')
        return true;
