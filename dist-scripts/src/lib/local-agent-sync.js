        ...
        for (const entry of entries) {
            ...
            if (stat.isFile() && entry.endsWith('.md') && entry !== 'CLAUDE.md' && entry !== 'AGENTS.md') {
                try {
                    ...
                    if (frontmatter.model)
                        configObj.model = frontmatter.model;
                    if (frontmatter.color)
                        configObj.color = frontmatter.color;
                    if (frontmatter.tools)
                        configObj.tools = frontmatter.tools;
                    if (frontmatter.description)
                        configObj.description = frontmatter.description;
                ...
                catch ( /* unreadable */_c) { /* unreadable */ }
                continue;
            }
            ...
            for (const f of IDENTITY_FILES) {
                ...
                if (existsSync(p)) {
                    ...
                    catch ( /* unreadable */_d) { /* unreadable */ }
                }
            }
            ...
            for (const f of CONFIG_FILES) {
                ...
                if (existsSync(p)) {
                    ...
                    catch ( /* unreadable */_e) { /* unreadable */ }
                }
            }
...
export async function syncLocalAgents() {
    try {
        ...
        const dbRows = db.prepare(`SELECT id, name, role, soul_content, status, source, content_hash, workspace_path, config FROM agents WHERE source = 'local'`).all();
        const dbMap = new Map();
 ...
 */
export function writeLocalAgentSoul(agentDir, soulContent) {
