...
function readPersisted() {
    try {
        if (!fs.existsSync(getGeneratedFilePath()))
            return {};
...
function writePersisted(values) {
    try {
        ...
        if (values.AUTH_SECRET)
            lines.push(`AUTH_SECRET=${values.AUTH_SECRET}`);
        if (values.API_KEY)
            lines.push(`API_KEY=${values.API_KEY}`);
 ...
 */
export function ensureAutoGeneratedCredentials() {
