...
interface CliTimeoutState {
  consecutiveTimeouts: number
  lastTimeoutAt: number
  disabledUntil?: number
}

function readCliState(): CliTimeoutState {
  try {
...
function writeCliState(state: CliTimeoutState): void {
  try {
...
function disableCliAcrossAgents(disabledUntilMs: number): void {
  ...
  for (const agentDir of agentDirs) {
    ...
    try {
      ...
      if (stats.disabledUntil && stats.disabledUntil >= disabledUntilMs) continue
 ...
 */
export function handleCliWatchdogTimeout(error: string): void {
  if (!error.includes('watchdog timeout') && !error.includes('noOutputTimeout') && !error.includes('cli exec')) return
  ...
  logger.warn(
    { consecutiveTimeouts: state.consecutiveTimeouts, threshold: CLI_TIMEOUT_THRESHOLD },
    'claude-cli watchdog timeout recorded'
  )
  ...
  if (state.consecutiveTimeouts >= CLI_TIMEOUT_THRESHOLD) {
    ...
    logger.warn(
      { disabledUntil: new Date(disabledUntilMs).toISOString(), cooldownHours: CLI_COOLDOWN_MS / 3600000 },
      `claude-cli disabled for ${CLI_COOLDOWN_MS / 3600000}h after ${CLI_TIMEOUT_THRESHOLD} consecutive watchdog timeouts`
    )
 ...
 */
const QUOTA_PATTERNS: Array<{ re: RegExp; extractMinutes?: boolean }> = [
...
interface QuotaInfo {
  ...
  retryAfterMs: number | null  // parsed hint, or null if unparseable
  provider: string | null       // e.g. "openai-codex", parsed from error context
}
 ...
 */
export function parseQuotaError(error: string): QuotaInfo {
  if (!error) return { isQuotaExhaustion: false, retryAfterMs: null, provider: null }
    ...
    if (match) {
      ...
      if (extractMinutes && match[1]) {
        ...
        if (minutes > 0 && minutes <= 1440) {
          retryAfterMs = Math.min(minutes * 60_000, MAX_DISABLE_MS)
        }
      }
      break
    }
  ...
  if (/chatgpt|openai|gpt-/i.test(error)) provider = 'openai-codex'
  else if (/anthropic|claude/i.test(error)) provider = 'anthropic'
  else if (/google|gemini/i.test(error)) provider = 'google'
  else if (/openrouter/i.test(error)) provider = 'openrouter'
 ...
 */
function findProfilesForProvider(profiles: Record<string, any>, provider: string): string[] {
  return Object.entries(profiles)
    .filter(([key, val]) => {
      ...
      if (val?.provider === provider) return true
      return false
    })
    .map(([key]) => key)
}
 ...
 */
export function disableProviderUntil(
  provider: string,
  disabledUntilMs: number,
  reason: string = 'rate_limit'
): { agentsUpdated: string[]; profilesDisabled: string[] } {
  const agentsUpdated: string[] = []
  const profilesDisabled: string[] = []
    ...
    try {
      ...
      for (const profileId of matchingProfiles) {
        ...
        if (stats.disabledUntil && stats.disabledUntil >= disabledUntilMs) {
          logger.info(
            { agentDir, profileId, existingUntil: new Date(stats.disabledUntil).toISOString() },
            'Provider already disabled until later — skipping'
          )
          continue
        }
 ...
 */
export function handleDispatchQuotaError(error: string): void {
  const quota = parseQuotaError(error)

  if (!quota.isQuotaExhaustion) return
  ...
  const durationMinutes = Math.round(disableDurationMs / 60_000)

  if (!quota.provider) {
    logger.warn(
      { error: error.substring(0, 200), durationMinutes },
      'Quota exhaustion detected but could not identify provider — cannot disable'
    )
    return
  }
