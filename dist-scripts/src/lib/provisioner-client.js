...
exports.runProvisionerCommand = runProvisionerCommand;
const net_1 = __importDefault(require("net"));
const DEFAULT_SOCKET = process.env.MC_PROVISIONER_SOCKET || '/run/mc-provisioner.sock';
async function runProvisionerCommand(input) {
...
function callProvisioner(payload) {
    return new Promise((resolve, reject) => {
        const socket = net_1.default.createConnection({ path: DEFAULT_SOCKET });
        let raw = '';
        ...
        socket.on('data', (chunk) => {
            ...
            if (idx !== -1) {
                ...
                try {
                    ...
                    reject(new Error(`Invalid provisioner response: ${err.message}`));
                }
            }
        });
        socket.on('error', (err) => {
