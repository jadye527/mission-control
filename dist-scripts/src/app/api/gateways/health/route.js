...
function ensureGatewaysTable(db) {
    db.exec(`
...
function parseGatewayVersion(res) {
...
function hasOpenClaw32ToolsProfileRisk(version) {
    if (!version)
        return false;
...
function buildGatewayProbeUrl(host, port) {
    const rawHost = String(host || '').trim();
    if (!rawHost)
        return null;
    const hasProtocol = rawHost.startsWith('ws://') ||
...
async function POST(request) {
    ...
    for (const gw of gateways) {
        ...
        if (h) {
            try {
                configuredHosts.add(new URL(h.includes('://') ? h : `http://${h}`).hostname);
            }
            catch (_a) {
                configuredHosts.add(h);
            }
        }
    }
    // Prepare update statements once (avoids N+1)
    const updateOnlineStmt = db.prepare("UPDATE gateways SET status = ?, latency = ?, last_seen = (unixepoch()), updated_at = (unixepoch()) WHERE id = ?");
    const updateOfflineStmt = db.prepare("UPDATE gateways SET status = ?, latency = NULL, updated_at = (unixepoch()) WHERE id = ?");
    const insertLogStmt = db.prepare("INSERT INTO gateway_health_logs (gateway_id, status, latency, probed_at, error) VALUES (?, ?, ?, ?, ?)");
    const results = [];
    for (const gw of gateways) {
        ...
        try {
            ...
            results.push(Object.assign({ id: gw.id, name: gw.name, status: status, latency, agents: [], sessions_count: 0, gateway_version: gatewayVersion, compatibility_warning: compatibilityWarning }, (errorMessage ? { error: errorMessage } : {})));
        }
        catch (err) {
    ...
    return server_1.NextResponse.json({ results, probed_at: Date.now() });
}
