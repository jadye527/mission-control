...
export async function spawnRecurringTasks() {
    ...
    `).all();
        ...
        for (const template of templates) {
            ...
            if (!(recurrence === null || recurrence === void 0 ? void 0 : recurrence.cron_expr) || !recurrence.enabled)
                continue;
            const lastSpawnedAtMs = recurrence.last_spawned_at ? recurrence.last_spawned_at * 1000 : 0;
            if (!isCronDue(recurrence.cron_expr, nowMs, lastSpawnedAtMs))
                continue;
      ...
      `).get(childTitle, template.workspace_id, template.project_id);
          ...
          `).run(template.project_id, template.workspace_id);
                ...
                const ticketRow = template.project_id
                    ? db.prepare(`SELECT ticket_counter FROM projects WHERE id = ? AND workspace_id = ?`).get(template.project_id, template.workspace_id)
                    : undefined;
        ...
        `).run(childTitle, template.description, template.assigned_to ? 'assigned' : 'inbox', template.priority, template.project_id, (_a = ticketRow === null || ticketRow === void 0 ? void 0 : ticketRow.ticket_counter) !== null && _a !== void 0 ? _a : null, template.assigned_to, 'scheduler', nowSec, nowSec, template.tags, JSON.stringify(childMetadata), template.workspace_id);
                const childId = Number(insertResult.lastInsertRowid);
                // Update template: bump spawn count and last_spawned_at
                const updatedRecurrence = Object.assign(Object.assign({}, recurrence), { last_spawned_at: nowSec, spawn_count: (recurrence.spawn_count || 0) + 1 });
                const updatedMetadata = Object.assign(Object.assign({}, metadata), { recurrence: updatedRecurrence });
                db.prepare(`
          UPDATE tasks SET metadata = ?, updated_at = ? WHERE id = ?
        `).run(JSON.stringify(updatedMetadata), nowSec, template.id);
                db_helpers.logActivity('task_created', 'task', childId, 'scheduler', `Recurring task spawned: ${childTitle}`, { parent_task_id: template.id, cron_expr: recurrence.cron_expr }, template.workspace_id);
            })();
            spawned++;
        }
