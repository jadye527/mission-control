...
async function GET(request) {
    ...
    `;
      ...
      WHERE workspace_id = ?
        AND ${commsPredicate}
    `;
        const messagesParams = [workspaceId];
        if (since) {
    ...
    `;
        ...
        const messages = db.prepare(messagesQuery).all(...messagesParams);
    ...
    `;
        const graphParams = [workspaceId, ...humanNames, ...humanNames];
        if (since) {
    ...
    `;
        const seededParams = [workspaceId, "conv-multi-%"];
        if (since) {
        ...
        const { seeded } = db.prepare(seededCountQuery).get(...seededParams);
        const seededCount = seeded || 0;
        const liveCount = Math.max(0, total - seededCount);
        const source = total === 0 ? "empty" :
            liveCount === 0 ? "seeded" :
                seededCount === 0 ? "live" :
                    "mixed";
        const parsed = messages.map((msg) => {
            ...
            return Object.assign(Object.assign({}, msg), { metadata: parsedMetadata });
        });
        ...
        logger_1.logger.error({ err: error }, "GET /api/agents/comms error");
        return server_1.NextResponse.json({ error: "Failed to fetch agent communications" }, { status: 500 });
    }
}
