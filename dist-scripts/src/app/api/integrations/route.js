...
let integrationProbeCache = null;
...
function getEnvPath() {
    if (!config_1.config.openclawStateDir)
        return null;
    return (0, path_1.join)(config_1.config.openclawStateDir, '.env');
}
...
async function writeEnvFile(lines) {
    ...
    await (0, promises_1.rename)(tmpPath, envPath);
}
...
function isPathLikeEnvVar(key) {
    return key.endsWith('_PATH') || key.endsWith('_FILE');
}
function isConfiguredValue(key, value) {
...
function checkOpAuthenticated(opEnv) {
    try {
        (0, child_process_1.execFileSync)('op', ['whoami', '--format', 'json'], {
            stdio: 'pipe',
            timeout: 3000,
...
function checkCommandAvailable(command) {
    try {
        (0, child_process_1.execFileSync)('which', [command], { stdio: 'pipe', timeout: 3000 });
        return true;
    }
    catch (_a) {
        return false;
...
function checkXintState() {
    ...
    const oauthPath = (0, path_1.join)(os_1.default.homedir(), '.xint', 'data', 'oauth-tokens.json');
    const envPath = (0, path_1.join)(os_1.default.homedir(), '.xint', '.env');
    const oauthConfigured = (0, fs_1.existsSync)(oauthPath);
    const envConfigured = (0, fs_1.existsSync)(envPath);
...
async function getIntegrationProbeSnapshot() {
    const now = Date.now();
...
function checkOpAvailable() {
    try {
        (0, child_process_1.execFileSync)('which', ['op'], { stdio: 'pipe', timeout: 3000 });
        return true;
    }
    catch (_a) {
        return false;
...
async function GET(request) {
    ...
    const envData = await readEnvFile();
    if (!envData) {
        return server_1.NextResponse.json({ error: 'OPENCLAW_STATE_DIR not configured' }, { status: 404 });
    }
    ...
    const providerSubscriptions = (0, provider_subscriptions_1.detectProviderSubscriptions)();
    // Merge plugin integrations and categories
    const pluginIntegrations = (0, plugins_1.getPluginIntegrations)();
    const allIntegrations = [...INTEGRATIONS];
    const pluginIntegrationMap = new Map();
    ...
    const allCategories = Object.assign({}, CATEGORIES);
    for (const pc of (0, plugins_1.getPluginCategories)()) {
    ...
    const integrations = allIntegrations.map(def => {
        ...
        if (def.id === 'onepassword' && !anySet && opAvailable) {
            const opEnv = Object.assign({}, process.env);
            const fileToken = envMap.get('OP_SERVICE_ACCOUNT_TOKEN');
            if (fileToken)
                opEnv.OP_SERVICE_ACCOUNT_TOKEN = fileToken;
        ...
        return {
            ...
            categoryLabel: (_b = (_a = allCategories[def.category]) === null || _a === void 0 ? void 0 : _a.label) !== null && _b !== void 0 ? _b : def.category,
            envVars: vars,
            status,
            vaultItem: (_c = def.vaultItem) !== null && _c !== void 0 ? _c : null,
            testable: (_d = def.testable) !== null && _d !== void 0 ? _d : false,
            recommendation: (_e = def.recommendation) !== null && _e !== void 0 ? _e : null,
        };
    });
...
async function PUT(request) {
    ...
    if (!(body === null || body === void 0 ? void 0 : body.vars) || typeof body.vars !== 'object') {
        return server_1.NextResponse.json({ error: 'vars object required' }, { status: 400 });
    }
    for (const key of Object.keys(body.vars)) {
        if (isVarBlocked(key)) {
            return server_1.NextResponse.json({ error: `Cannot set protected variable: ${key}` }, { status: 403 });
        }
        if (!/^[A-Z_][A-Z0-9_]*$/i.test(key)) {
            return server_1.NextResponse.json({ error: `Invalid variable name: ${key}` }, { status: 400 });
        }
    }
    const envData = await readEnvFile();
    if (!envData) {
    ...
    return server_1.NextResponse.json({ updated: updatedKeys, count: updatedKeys.length });
}
// ---------------------------------------------------------------------------
...
async function DELETE(request) {
    ...
    if (!keysParam) {
        return server_1.NextResponse.json({ error: 'keys parameter required (comma-separated string or array)' }, { status: 400 });
    }
    const keysToRemove = new Set(keysParam.split(',').map((k) => k.trim()).filter(Boolean));
    if (keysToRemove.size === 0) {
        return server_1.NextResponse.json({ error: 'At least one key required' }, { status: 400 });
    }
    for (const key of keysToRemove) {
        if (isVarBlocked(key)) {
            return server_1.NextResponse.json({ error: `Cannot remove protected variable: ${key}` }, { status: 403 });
        }
    }
    const envData = await readEnvFile();
    if (!envData) {
    ...
    return server_1.NextResponse.json({ removed, count: removed.length });
}
// ---------------------------------------------------------------------------
...
async function POST(request) {
    ...
    if (rateCheck)
        return rateCheck;
    const result = await (0, validation_1.validateBody)(request, validation_1.integrationActionSchema);
    if ('error' in result)
        return result.error;
    ...
    if (!body.integrationId) {
        return server_1.NextResponse.json({ error: 'integrationId required' }, { status: 400 });
    }
    let integration = INTEGRATIONS.find(i => i.id === body.integrationId);
    if (!integration) {
        // Check plugin integrations
        const pi = (0, plugins_1.getPluginIntegrations)().find(i => i.id === body.integrationId);
        if (pi) {
            integration = {
                id: pi.id,
                name: pi.name,
    ...
    if (!integration) {
        return server_1.NextResponse.json({ error: `Unknown integration: ${body.integrationId}` }, { status: 404 });
    }
    if (body.action === 'test') {
    ...
    return server_1.NextResponse.json({ error: `Unknown action: ${body.action}` }, { status: 400 });
}
// ---------------------------------------------------------------------------
// Test connection for an integration
// ---------------------------------------------------------------------------
async function handleTest(integration, request, user) {
    var _a;
    if (!integration.testable) {
        return server_1.NextResponse.json({ error: 'This integration does not support testing' }, { status: 400 });
    }
    const envData = await readEnvFile();
    if (!envData) {
        return server_1.NextResponse.json({ error: 'OPENCLAW_STATE_DIR not configured' }, { status: 404 });
    ...
    try {
        ...
        switch (integration.id) {
            case 'telegram': {
                ...
                if (!token)
                    return server_1.NextResponse.json({ ok: false, detail: 'Token not set' });
            ...
            case 'anthropic': {
                ...
                if (!key) {
                    ...
                    if (sub)
                        return server_1.NextResponse.json({ ok: true, detail: `OAuth/subscription detected: ${sub.type}` });
                    return server_1.NextResponse.json({ ok: false, detail: 'API key not set' });
                }
            ...
            case 'google_workspace': {
                ...
                try {
                    ...
                    if (credsFile)
                        env.GOOGLE_WORKSPACE_CLI_CREDENTIALS_FILE = credsFile;
                    (0, child_process_1.execFileSync)('gws', ['auth', 'status'], {
                        timeout: 10000,
                    ...
                    const stderr = ((_a = err.stderr) === null || _a === void 0 ? void 0 : _a.toString()) || '';
            ...
            default: {
                ...
                const pluginDef = (0, plugins_1.getPluginIntegrations)().find(pi => pi.id === integration.id);
                if (pluginDef === null || pluginDef === void 0 ? void 0 : pluginDef.testHandler) {
                ...
                if (url) {
                    ...
                    return server_1.NextResponse.json({ ok: false, detail: 'No test available — configure the integration URL to enable testing' });
                }
                break;
            }
        }
        ...
        return server_1.NextResponse.json({ ok: false, detail: err.message || 'Connection failed' });
    }
}
// ---------------------------------------------------------------------------
// Pull value from 1Password vault — uses execFileSync (no shell) for safety
// ---------------------------------------------------------------------------
async function handlePull(integration, request, user) {
    if (!integration.vaultItem) {
        return server_1.NextResponse.json({ error: 'No vault item configured for this integration' }, { status: 400 });
    }
    if (!checkOpAvailable()) {
        return server_1.NextResponse.json({ error: '1Password CLI (op) is not installed' }, { status: 400 });
    }
    try {
        const opEnv = await getOpEnv();
        if (!opEnv.OP_SERVICE_ACCOUNT_TOKEN) {
            return server_1.NextResponse.json({ error: 'OP_SERVICE_ACCOUNT_TOKEN not found in environment or .env' }, { status: 400 });
        }
        // execFileSync passes args as array — no shell interpolation possible
        const secret = (0, child_process_1.execFileSync)('op', [
        ...
        if (!value || value.length === 0) {
            return server_1.NextResponse.json({ error: 'Empty value returned from 1Password' }, { status: 400 });
        }
        // Write to .env
        const envData = await readEnvFile();
        if (!envData) {
...
// ---------------------------------------------------------------------------
async function handlePullAll(request, user, category) {
    if (!checkOpAvailable()) {
        return server_1.NextResponse.json({ error: '1Password CLI (op) is not installed' }, { status: 400 });
    }
    const opEnv = await getOpEnv();
    ...
    if (targets.length === 0) {
        return server_1.NextResponse.json({ error: 'No vault-backed integrations found for this category' }, { status: 400 });
    }
    const envData = await readEnvFile();
    if (!envData) {
        return server_1.NextResponse.json({ error: 'OPENCLAW_STATE_DIR not configured' }, { status: 404 });
    ...
    (0, db_1.logAuditEvent)({
        ...
        actor_id: user.id,
        detail: {
            category: category !== null && category !== void 0 ? category : 'all',
