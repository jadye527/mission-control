...
const MODEL_PRICING = {
    ...
    'claude-haiku-4-5': { input: 0.8 / 1000000, output: 4 / 1000000 },
};
const DEFAULT_PRICING = { input: 3 / 1000000, output: 15 / 1000000 };
...
async function parseSessionFile(filePath, projectSlug, fileMtimeMs, fileSizeBytes) {
    var _a, e_1, _b, _c;
    try {
        ...
        if (!hasLines || !sessionId || (userMessages === 0 && assistantMessages === 0))
            return null;
    ...
    `);
        ...
        db.transaction(() => {
            ...
            for (const s of sessions) {
                upsert.run(s.sessionId, s.projectSlug, s.projectPath, s.model, s.gitBranch, s.userMessages, s.assistantMessages, s.toolUses, s.inputTokens, s.outputTokens, s.estimatedCost, s.firstMessageAt, s.lastMessageAt, s.lastUserPrompt, s.isActive ? 1 : 0, nowSec, nowSec);
                upserted++;
            }
        })();
