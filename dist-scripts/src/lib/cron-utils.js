...
exports.describeCronFrequency = describeCronFrequency;
exports.validateCronExpression = validateCronExpression;
exports.generateCloneName = generateCloneName;
    ...
    for (let i = 0; i < 5; i++) {
        ...
        if (field.startsWith('*/')) {
            ...
            if (isNaN(step) || step < 1)
                return `Invalid step value in ${fieldNames[i]}: ${field}`;
            continue;
        }
        ...
        for (const segment of segments) {
            ...
            for (const part of rangeParts) {
                ...
                if (isNaN(num))
                    return `Invalid value in ${fieldNames[i]}: ${part}`;
...
function generateCloneName(name, existingNames) {
