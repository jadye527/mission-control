...
async function GET(request) {
    ...
    if (!type || !['audit', 'tasks', 'activities', 'pipelines'].includes(type)) {
        return server_1.NextResponse.json({ error: 'type required: audit, tasks, activities, pipelines' }, { status: 400 });
    }
    ...
    // JSON format
    return server_1.NextResponse.json({ type, exported_at: new Date().toISOString(), count: rows.length, data: rows }, {
        headers: {
