...
async function POST(request) {
    ...
    `);
        ...
        const standupData = agents.map(agent => {
            ...
            const activityCount = activityCountStmt.get(agent.name, workspaceId, startOfDay, endOfDay);
            const commentsToday = commentCountStmt.get(agent.name, workspaceId, startOfDay, endOfDay);
            return {
                agent: {
                    name: agent.name,
                    role: agent.role,
        ...
        const teamAccomplishments = standupData
            .flatMap(agent => agent.completedToday.map(task => (Object.assign(Object.assign({}, task), { agent: agent.agent.name }))))
            .sort((a, b) => b.updated_at - a.updated_at);
        const teamBlockers = standupData
            .flatMap(agent => agent.blocked.map(task => (Object.assign(Object.assign({}, task), { agent: agent.agent.name }))))
            .sort((a, b) => {
    ...
    `).run(targetDate, JSON.stringify(standupReport), createdAt, workspaceId);
        ...
        db_1.db_helpers.logActivity('standup_generated', 'standup', 0, // No specific entity
        auth.user.username, `Generated daily standup for ${targetDate}`, {
            date: targetDate,
        ...
        return server_1.NextResponse.json({ standup: standupReport });
    }
    catch (error) {
        logger_1.logger.error({ err: error }, 'POST /api/standup/generate error');
        return server_1.NextResponse.json({ error: 'Failed to generate standup' }, { status: 500 });
    }
}
/**
    ...
    `).all(workspaceId, limit, offset);
        const standupHistory = standupRows.map((row, index) => {
            ...
            return {
                ...
                agentCount: ((_a = report.summary) === null || _a === void 0 ? void 0 : _a.totalAgents) || 0
            };
        });
        const countRow = db
        ...
        logger_1.logger.error({ err: error }, 'GET /api/standup/history error');
        return server_1.NextResponse.json({ error: 'Failed to fetch standup history' }, { status: 500 });
    }
}
