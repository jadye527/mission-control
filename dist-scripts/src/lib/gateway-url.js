...
exports.buildGatewayWebSocketUrl = buildGatewayWebSocketUrl;
...
function preserveTokenQuery(parsed) {
...
function formatWebSocketUrl(parsed) {
    return parsed.toString().replace(/\/$/, '').replace('/?', '?');
}
function buildGatewayWebSocketUrl(input) {
    ...
    const prefixed = rawHost.startsWith('ws://') ||
    ...
    const shouldOmitPort = wsProtocol === 'wss' &&
        !isLocalHost(rawHost) &&
        port === 18789;
